...
class TestInstantiation:
    ...
    def test_from_json(self, valid_data):
        ...
        sensor_from_dict = SensorBase(**valid_data)
...
class TestErrorCalculation:
    ...
    def test_calculate_error_add(self, valid_data, measured_value, expected_error):
    ...
    def test_calculate_error_max(self, valid_data, measured_value, expected_error):
        """Verify error calculation with 'max' function"""
        for acc in valid_data['accuracies']:
            acc['error_combination'] = 'max'
        sensor = SensorBase(**valid_data)
        assert sensor.calculate_error(measured_value) == pytest.approx(expected_error)

    def test_calculate_error_out_of_bounds(self, valid_data):
    ...
    def test_accuracies_sorting(self, valid_data):
        """Verify accuracies sorted correctly"""
        valid_data['accuracies'].reverse()  # Reverse accuracies sequence of input
        sensor = SensorBase(**valid_data)
        assert sensor.accuracies[0].range == (0, 5000)
